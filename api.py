...
class RiotAPI:
	...
	def _base_query_old_old(self, db_collection, url, items):
	...
	def _base_query_old(self, db_collection, url_left, url_right, items):
		'''
		Base query that the API uses.  
		'''
		data = []
		call_items = []
		dbUpdate = []
	...
	def _base_query_multi(self, db_collection, url_left, url_right, items):
		...
		the url of the external API that we need to make a GET request to.  The left and right sides of this are because the variable part of the URL that we're getting the data from may occur in the middle of the URL, e.g. /championmastery/location/na/player/{playerid}/champions.  Here url_left is everything before {playerid} and url_right is everything after.  NB: There are more sophisticated ways of doing this

		items : List[str]

		A list of items that the query wants to get (up to 40), e.g. info for a list of 40 different usernames.  Some calls to the external Riot API permit comma-separated lists to retrieve multiple values at once; I replicate this functionality but allowing the items here to be a list.  
	...
	def _base_query_single(self, db_collection, url_left, url_right, item):
		...
		An item that the query wants to get, e.g. info about a match with a given id.  Most calls to the external Riot API only permit a single input; this function handles those cases.
		'''
	...
	def _get_call_items(self, db_collection, items):
		'''
		Given a list of multiple pieces of data the user wants, some of these may already exist in the DB and some may not.  This function sorts the incoming requests into three categories: data not in the DB, data that is in the DB but determined to be stale, and data that is in the DB and determined to not be stale.  The former two categories are returned to the base query to become an API call, while the latter is just returned as data.  
		...
		A list of strings corresponding to data that does not exist in the DB  (or does exist and is stale) and needs to be handed off to the external API.
	...
	def _get_call_item_single(self, db_collection, item):
		'''
		Analogous to _get_call_items, but for only one item, which simplifies the task considerably.  Given a request for data, this may already exist in the DB or it may not.  This function places the incoming request into one of three categories: data not in the DB, data that is in the DB but determined to be stale, and data that is in the DB and determined to not be stale.  The former two categories are returned to the base query to become an API call, while the latter is just returned as data.  
		...
		NB: Only one return value for this function will be meaningful for any particular call.  This makes the code structure parallel to the multiple-query case at the cost of being much less intuitive in the single-query case.  Given more time, this is something that I'd work on.

		call_item : str

		A string corresponding to data that does not exist in the DB  (or does exist and is stale) and needs to be handed off to the external API.
	...
	def get_match(self, matchid):
		...
		return self._base_query_single(db_collection, url, playerid)
